...
export const Create_Order = AsyncWrapper(async (req, res, next) => {
  ...
  const isCourseValid = courses.some((course)=>course?._id.toString() === courseId)
  if(!isCourseValid){
    return next(customError(400,'Course does not exist'))
  }
  const CourseDetail = await CourseModel.findById(courseId).select('name price estimatedPrice ')
  const orderData = {
    courseId,
    payment_info,
    userId: user?._id,
  ...
  const template = await ejs.renderFile(path.join(__dirname,'../Templates/newOrderEmail.ejs'),{
    userName:user?.name,
